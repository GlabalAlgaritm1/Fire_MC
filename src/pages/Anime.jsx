...
import { AnimeData, images } from '../data'
import { Swiper, SwiperSlide } from 'swiper/react'
import 'swiper/css'
import Marquee from 'react-fast-marquee'

const Anime = () => {
  const firstRow = [...AnimeData.slice(0, 12), { id: 'see-all-1', seeAll: true }]
  const secondRow = [...AnimeData.slice(12, 24), { id: 'see-all-2', seeAll: true }]
  ...
  const renderCard = (item) => (
    <Link to={`/anime/${item.id}`} key={item.id}>
      <li className="relative rounded overflow-hidden shadow-lg group cursor-pointer">
        ...
        <span className="text-sm font-semibold text-orange-600">{item.buy}</span>
      </li>
    </Link>
  )
  ...
  const renderSeeAll = (id) => (
    <li
      key={id}
      className="flex flex-col items-center justify-center border border-white/20 rounded-lg hover:bg-white/20 transition-all duration-300 cursor-pointer h-[275px]"
    >
      <Link to="/anime" className="flex flex-col items-center justify-center gap-y-3 w-full h-full">
    ...
    <section>
      <div className="mt-4">
        <div className="flex items-center justify-between py-3 sticky top-[56px] z-40 bg-[#171818]">
          <Link className="text-2xl font-semibold">Animelar <span className="text-white">:</span></Link>
        </div>
