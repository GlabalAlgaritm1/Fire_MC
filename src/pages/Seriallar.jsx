...
const Seriallar = () => {
  // 🔹 1-qator (birinchi 12 ta + see-all)
  const firstRow = [
    ...SerialData.slice(0, 12),
    { id: 'see-all-1', seeAll: true }
  ]

  // 🔹 2-qator (keyingi 12 ta + see-all)
  const secondRow = [
    ...SerialData.slice(12, 24),
    { id: 'see-all-2', seeAll: true }
  ]

  // 🔹 Card UI
  const renderCard = (item) => (
    <Link to={`/serial/${item.id}`}>
      ...
      <div className="mt-4">
        ...
        <div className="flex items-center justify-between py-3 sticky top-[56px] z-40 bg-[#171818]">
          <Link className="text-2xl font-semibold">
            Seriallar <span className="text-white">:</span>
          </Link>
        </div>
