...
import { SerialData, images } from '../data'
import { Swiper, SwiperSlide } from 'swiper/react'
import 'swiper/css'
import Marquee from 'react-fast-marquee'

const Seriallar = () => {
  const firstRow = [...SerialData.slice(0, 12), { id: 'see-all-1', seeAll: true }]
  const secondRow = [...SerialData.slice(12, 24), { id: 'see-all-2', seeAll: true }]

  const renderCard = (item) => (
    <Link to={`/serial/${item.id}`} key={item.id}>
  ...
  const renderSeeAll = (id) => (
    ...
    >
      <Link to="/serial" className="flex flex-col items-center justify-center gap-y-3 w-full h-full">
    ...
    <section>
      <div className="mt-4">
        <div className="flex items-center justify-between py-3 sticky top-[56px] z-40 bg-[#171818]">
          <Link className="text-2xl font-semibold">Seriallar <span className="text-white">:</span></Link>
        </div>
