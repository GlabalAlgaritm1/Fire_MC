...
const AllAnime = () => {
  // 🔹 AnimeData teskari tartibda va 72 taga cheklangan
  const reversedAnimes = [...AnimeData].reverse().slice(0, 72)
  ...
  const isPremyera = (addedAt) => {
    ...
    const now = new Date()
    const AnimeDate = new Date(addedAt)
    return now - AnimeDate <= PREMIERA_MS
  }

  // 🔹 Ko‘proq ko‘rsatish tugmasi
  const handleShowMore = () => {
    setVisibleCount((prev) => Math.min(prev + 18, reversedAnimes.length))
  }
    ...
    <section className="p-6">
      <div className="flex items-center justify-between py-3 sticky top-[56px] z-40 bg-[#171818]">
        <h1 className="text-2xl font-semibold text-white">
          Barcha Animelar <span className="text-green-500">:</span>
        </h1>
      </div>

      {/* Grid layout */}
      <ul className="grid grid-cols-5 gap-6 mt-6">
        {reversedAnimes.slice(0, visibleCount).map((item) => (
      ...
      {visibleCount < reversedAnimes.length && (
        <button
