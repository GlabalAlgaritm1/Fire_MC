...
const AllSerial = () => {
  // 🔹 SerialData teskari tartibda va 72 taga cheklangan
  const reversedSerials = [...SerialData].reverse().slice(0, 72)
  ...
  const isPremyera = (addedAt) => {
    ...
    const now = new Date()
    const SerialDate = new Date(addedAt)
    return now - SerialDate <= PREMIERA_MS
  }

  // 🔹 Ko‘proq ko‘rsatish tugmasi
  const handleShowMore = () => {
    setVisibleCount((prev) => Math.min(prev + 18, reversedSerials.length))
  }
    ...
    <section className="p-6">
      <div className="flex items-center justify-between py-3 sticky top-[56px] z-40 bg-[#171818]">
        <h1 className="text-2xl font-semibold text-white">
          Barcha Seriallar <span className="text-green-500">:</span>
        </h1>
      </div>

      {/* Grid layout */}
      <ul className="grid grid-cols-5 gap-6 mt-6">
        {reversedSerials.slice(0, visibleCount).map((item) => (
      ...
      {visibleCount < reversedSerials.length && (
        <button
